...
def _serialize_license(record: License, now: datetime | None = None) -> dict:
    ...
    return {
        ...
        "plan": record.plan,
        "status": record.effective_status(current).value,
        "starts_at": record.starts_at.isoformat(),
        "expires_at": record.expires_at.isoformat(),
        "activated_at": record.activated_at.isoformat() if record.activated_at else None,
        "last_renewal_at": record.last_renewal_at.isoformat() if record.last_renewal_at else None,
        "renewal_count": record.renewal_count,
        "last_access_at": record.last_access_at.isoformat() if record.last_access_at else None,
        "revoked_at": record.revoked_at.isoformat() if record.revoked_at else None,
        "price_cents": record.price_cents,
        "entitlements": {
            "route_optimization": record.entitlements.route_optimization,
            "max_devices": record.entitlements.max_devices,
            "max_routes_per_day": record.entitlements.max_routes_per_day,
        },
    }
...
def register_admin_routes(
    ...
    *,
    ...
    def require_admin(authorization: str | None):
        ...
        if authenticated.account.role != AccountRole.ADMIN:
    ...
    def transition(call):
        try:
            ...
            message = str(exc)
            status = 404 if message == "license not found" else 409
    ...
    def list_licenses(
        ...
        if status:
            records = [item for item in records if item.effective_status(now) == status]
    ...
    def get_license(license_id: str, authorization: str | None = Header(default=None)) -> dict:
        require_admin(authorization)
        record = licenses.get_by_id(license_id)
        if record is None:
    ...
    def license_history(license_id: str, authorization: str | None = Header(default=None)) -> dict:
        require_admin(authorization)
        if licenses.get_by_id(license_id) is None:
            raise HTTPException(status_code=404, detail="License not found")
    ...
    def revoke_device(device_id: str, authorization: str | None = Header(default=None)) -> dict:
        ...
        if device.revoked_at is not None:
            raise HTTPException(status_code=409, detail="Device is already revoked")
