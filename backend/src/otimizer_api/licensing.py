...
class Entitlements:
    ...
    def __post_init__(self) -> None:
        if self.max_devices < 1: raise ValueError("max_devices must be at least 1")
        if self.max_routes_per_day is not None and self.max_routes_per_day < 1: raise ValueError("max_routes_per_day must be positive when configured")
...
class License:
    ...
    plan: str = "default"
    def __post_init__(self) -> None:
        if not self.license_id.strip(): raise ValueError("license_id must not be empty")
        if not self.account_id.strip(): raise ValueError("account_id must not be empty")
        if not self.license_key.strip(): raise ValueError("license_key must not be empty")
        if self.price_cents < 0: raise ValueError("price_cents must be non-negative")
        if self.renewal_count < 0: raise ValueError("renewal_count must be non-negative")
        if self.status == LicenseStatus.REVOKED and self.revoked_at is None: raise ValueError("revoked licenses require revoked_at")
    def change_price(self, price_cents: int) -> "License":
        if price_cents < 0: raise ValueError("price_cents must be non-negative")
    ...
    def effective_status(self, now: datetime | None = None) -> LicenseStatus:
        current = _utc(now)
        if self.status == LicenseStatus.REVOKED or self.revoked_at is not None: return LicenseStatus.REVOKED
        if self.status == LicenseStatus.SUSPENDED: return LicenseStatus.SUSPENDED
        if current >= self.expires_at: return LicenseStatus.EXPIRED
        return self.status
...
class InMemoryLicenseRepository:
    def __init__(self, licenses: list[License] | None = None): self._licenses = {item.license_id: item for item in licenses or []}
    def save(self, license_record: License) -> None: self._licenses[license_record.license_id] = license_record
    def get_by_id(self, license_id: str) -> License | None: return self._licenses.get(license_id)
    def get_active_license(self, account_id: str, now: datetime) -> License | None:
...
class InMemoryLicenseEventRepository:
    def __init__(self, events: list[LicenseEvent] | None = None): self._events = list(events or [])
    def append(self, event: LicenseEvent) -> None: self._events.append(event)
    def list_for_license(self, license_id: str) -> list[LicenseEvent]: return [event for event in self._events if event.license_id == license_id]


def generate_license_key() -> str: return token_urlsafe(32)
...
class LicenseAuthorizer:
    def __init__(self, repository: LicenseRepository): self.repository = repository
    def authorize_route(self, account_id: str, now: datetime | None = None) -> LicenseDecision:
        current = _utc(now); license_record = self.repository.get_active_license(account_id, current)
        if license_record is None: return LicenseDecision(False, "LICENSE_REQUIRED", "A valid license is required to generate a route.")
        if not license_record.entitlements.route_optimization: return LicenseDecision(False, "FEATURE_NOT_ENTITLED", "The current plan does not allow route optimization.", license_record)
...
class LicenseLifecycleService:
    ...
    def __init__(self, repository: LicenseRepository, events: LicenseEventRepository): self.repository, self.events = repository, events
    ...
    def renew(self, license_id: str, actor_account_id: str, duration: timedelta, now: datetime | None = None) -> License:
        current = _utc(now)
        if duration <= timedelta(0): raise LicenseLifecycleError("renewal duration must be positive")
    ...
    def reactivate(self, license_id: str, actor_account_id: str, now: datetime | None = None) -> License:
        current = _utc(now); record = self._get(license_id)
        if record.status != LicenseStatus.SUSPENDED: raise LicenseLifecycleError("only suspended licenses can be reactivated")
        if current >= record.expires_at: raise LicenseLifecycleError("expired license cannot be reactivated; renew it first")
    ...
    def revoke(self, license_id: str, actor_account_id: str, reason: str, now: datetime | None = None) -> License:
        if not reason.strip(): raise LicenseLifecycleError("revocation reason is required")
        current = _utc(now); record = self._get(license_id)
        if record.status == LicenseStatus.REVOKED or record.revoked_at is not None: raise LicenseLifecycleError("license is already revoked")
        updated = replace(record, status=LicenseStatus.REVOKED, revoked_at=current)
        self._commit(updated, create_license_event(license_id, "REVOKED", current, actor_account_id=actor_account_id, previous_status=record.status, new_status=LicenseStatus.REVOKED, reason=reason)); return updated
    def expire(self, license_id: str, now: datetime | None = None) -> License:
        current = _utc(now); record = self._get(license_id)
        if record.status == LicenseStatus.REVOKED: raise LicenseLifecycleError("revoked license cannot be expired")
        if current < record.expires_at: raise LicenseLifecycleError("license has not reached expiration")
        if record.status == LicenseStatus.EXPIRED: return record
        updated = replace(record, status=LicenseStatus.EXPIRED)
        self._commit(updated, create_license_event(license_id, "EXPIRED", current, previous_status=record.status, new_status=LicenseStatus.EXPIRED)); return updated
    def _transition(self, license_id: str, actor_account_id: str, action: str, new_status: LicenseStatus, now: datetime | None, *, allowed: set[LicenseStatus], reason: str | None = None, set_activated: bool = False) -> License:
        current = _utc(now); record = self._get(license_id)
        if record.effective_status(current) == LicenseStatus.EXPIRED: raise LicenseLifecycleError("expired license must be renewed before activation")
        if record.status not in allowed: raise LicenseLifecycleError(f"transition {record.status.value} -> {new_status.value} is not allowed")
